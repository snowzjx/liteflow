...
static int rx_msg(struct nl_msg *msg, void* args)
{
    ...
    nn_input = nla_data(attr[LF_TCP_NL_ATTR_NN_INPUT]);
    wrapper->shoud_stop = wrapper->ops(nn_input, INPUT_SIZE);

    return 0;
}

int rx_tcp_dp_notification(bool (*rx)(__s64 *nn, size_t size))
{
    ...
    family_id = resolve_family_id(sock, LF_TCP_NL_NAME);
    if (family_id < 0) {
        return -1;
    }

    grp_id = resolve_grp_id(sock, LF_TCP_NL_NAME, LF_TCP_NL_MC_DEFAULT_NAME);
    if (grp_id < 0) {
